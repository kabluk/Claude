...
def handoff_prompt() -> str:
    ...
    Открывается вызовом /project-orchestrator — владелец (2026-08-13) явно
    попросил, чтобы КАЖДАЯ новая сессия начиналась с оркестратора, а не с
    ручного чтения файлов по одному; сам оркестратор читает HANDOFF.md/
    STATUS.md на своём Шаге 1, так что порядок "сначала прочитай файлы, потом
    вызови оркестратор" был лишним шагом. Дальше — ссылка на HANDOFF.md, а не
    пересказ: пересказ устаревает в момент написания, ссылка — нет (тот же
    принцип, что single source of truth, LEARNING_LOG 2026-08-07).
    """
    ...
    lines = [
        "/project-orchestrator Продолжи проект AccessAtlas (ветка " + branch + ",",
        "от accessatlas — НИКОГДА не от main, там чужой проект detnav). Прочитай",
        "сам docs/project/HANDOFF.md и STATUS.md на своём Шаге 1 — не пересказываю",
        "их здесь, чтобы не устареть в момент написания. Выбери следующий",
        "незаблокированный узел из GRAPH.yaml (или ЗАДАЧУ ниже, если она указана",
        "явно), выполни через /task-loop, обнови handoff в конце итерации.",
        "",
        "Проверка среды: npm install && node scripts/build-a11y.mjs — ожидать 574 агентства.",
        "",
        "ЗАДАЧА: <впиши задачу, если конкретная — иначе оркестратор продолжит по графу>",
    ]
    if head:
