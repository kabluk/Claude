...
- ✅ **Дни 24–27 (декларации гос-сайтов ЕС — защищаемый датасет):** 4 агента извлекли **+99 агентств**, названных аудиторами в обязательных декларациях доступности: FR 26, PL 26, NL 17, DE 18, AT 4, BE 5, ES 2, IT 1. Правило «оба факта или мимо» (имя в декларации + живой сайт с услугами); отсеяно ~150 кандидатов. Методы: обход 5801 страницы реестра NL + разбор 1569 PDF-отчётов (в реестре аудитор обезличен); регулярка по шаблону польских деклараций (~450 страниц/PDF); массовый обход ~500 гос-доменов DE/AT + отчёты BITV-Test. Firecrawl использован экономно (~2600 кредитов на всё), основная масса — бесплатный curl.
  - Добавлен `data/a11y/excluded.json` — «надгробия»: merge-a11y больше не воскрешает осознанно удалённые записи из старых партий сборщиков.
  - **Каталог: 254 агентства, 21 страна, 75 индексируемых списочных страниц, sitemap 347 URL.** Рост по ключевым рынкам: FR 3→29, PL 3→29, NL 10→27, DE 24→42.
  - Структурное ограничение (не недоработка): IT и ES бедны — форма AgID фиксирует «оценку третьей стороной» без имени, испанские декларации в массе «самооценка». Чехия/Словакия не понадобились — польская жила не иссякла.

**Следующий шаг (по 30-дневному плану):**
1. **Дни 21–23 (единственное, что блокирует запуск):** купить домен → заменить заглушку origin (`a11y-site/src/lib/seo.tsx` + `scripts/gen-a11y-sitemap.mjs`), деплой на Cloudflare Pages, лид-форма (Worker + D1), Stripe Payment Links для featured, claim-outreach по базе (у 99 новых агентств есть готовый повод для письма: «вас назвали аудитором в декларации X — заберите профиль»).
2. **Дни 28–30:** финальная проверка индексации, Search Console, обогащение описаний для 99 новых записей (сейчас у них пустой `description`).
