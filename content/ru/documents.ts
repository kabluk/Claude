...
const c: PageContent = {
  ...
  blocks: [
    ...
    },
    {
      kind: 'callout',
      tone: 'n',
      title: 'Детейнер (ICE hold) — если человек ещё в местной тюрьме',
      body: [
        'Детейнер — это просьба ICE к местной тюрьме подержать человека до 48 часов после того, как его должны были отпустить, чтобы ICE успел его забрать. Это просьба, а не ордер.',
        'Тюрьма не обязана её исполнять, а если ICE не приезжает за 48 часов (выходные и праздники считаются) — человека должны освободить. Это вопрос к адвокату по уголовному делу.',
      ],
    },
