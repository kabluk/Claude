...
## Обновление (воспроизводимо)

Данные deportationdata.org обновляются ~раз в месяц. Раз в квартал:

    pip install pyarrow
    python3 scripts/build-directory.py path/to/facilitieslatest.parquet

Скрипт применяет тот же фильтр и перезаписывает data/directory.json.
Проверено: на текущем parquet воспроизводит те же 196 учреждений
идентично. После обновления — сменить дату в
content/{ru,en,es}/ui.ts (visitFinder.provenance) и здесь.

Аделанто — проверенная вручную карточка (data/facilities.json,
code ADLNTCA) — показывается поверх директории с реальными
правилами свиданий.
