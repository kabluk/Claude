...
| CN-RESEARCH | Регулярные data-продукты (§23, §45) на уникальных данных — из САМОГО каталога, не бенчмарк чужих сайтов (корпуса сканов нет, D-010). **Первый кусок done (D-071): `/reports/verified-audit-market`** (agencies.json) — числа считаются `scripts/reports-data.mjs`, гейт `reports-data.test.mjs`. **Второй кусок done (D-085, 2026-08-08): `/reports/en301549-automation-coverage/`** (en301549-coverage.json, свежий источник) — `scripts/en301549-report-data.mjs`, гейт `en301549-report-data.test.mjs`; `src/lib/reports.ts`/`ReportDocPage.tsx` рефакторены на диспетчер по slug, первый отчёт не изменился (проверено побайтово). Деплой НЕ требуется | growth + data | — | in progress (2 отчёта; следующий — новый data-продукт по решению направления) |
