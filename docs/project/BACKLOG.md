...
| G-GUIDES | Новые гайды под кластеры BFSG/EAA/VPAT | **in progress** (D-056): выбран product-lead как более ценный прямо сейчас (контент растит трафик и без реального домена, hreflang — нет). Первый кусок — **done**: гайд `barrierefreiheitserklaerung-bfsg-anlage3` (пять пунктов Anlage 3 zu §14 BFSG, узел `G-GUIDES-BFSG-STATEMENT` в GRAPH.yaml). Остаток НЕ начат: EAA-кластер для 8 юрисдикций D-031 (IT/IE/AT/BE/SE/DK/FI/NO — ни одной без гайда), VPAT-кластер (сравнение с EN 301 549 отчётом, факты не проверены), ещё BFSG-контент |
