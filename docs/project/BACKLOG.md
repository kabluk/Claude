...
| `R-HEALTH-CRON` | Ежедневный health-check прода из УЖЕ существующего worker-cron (`0 3 * * *`, `worker/index.js::scheduled`) + письмо владельцу через Resend при поломке. Ловит то, что происходит МЕЖДУ деплоями: протухший токен, упавший воркер, DNS, D1 | backend-engineer | todo. Прим.: GitHub Actions `schedule` здесь НЕ годится — расписания читаются только из ветки по умолчанию, а `main` этого репо занят другим проектом |
