...
## Фаза R — надёжность и качество

| ID | Задача | Владелец | Зависит от | Статус |
|---|---|---|---|---|
| G-FOLD-GATE | Гейт `check-fold`: главный ответ инструмента виден на телефоне без прокрутки (D-187) | qa-engineer | G-CHECKERS-ELEVATION, G-CHECKER-IMAGEPICKER | **done** |
| G-SCAN-REPORT-ELEVATION | `.panel`/`.result-hero` на `/scan/` и `/report/:id` (последние две плоские `.card`-страницы) + `check-fold` расширен на `/report/:id` | frontend-engineer | G-CHECKERS-ELEVATION, G-FOLD-GATE | **done** |
...
| A0-SOURCEREF-LANG | У 24 агентств в `sourceRefs[].label` — русские служебные пометки верификации («checked … пакеты 11-15», «по архивной копии…»), и AgencyPage рендерит их пользователю (найдено при D-105). Вычистить мета-комментарии из label (цитаты и даты сохранить), НЕ выдумывая содержимого; проверить и `foundedNote`-подобные поля | data-engineer | — | **done** (D-111, 2026-08-10: 24/24 записи очищены/переведены, факты и цитаты не тронуты; глубокий обход всего `agencies.json` — 0 кириллицы в любом поле, отдельного `foundedNote`-поля в схеме нет) |
...
| A1-REPORT-DIRECT-LINK | **Прямая ссылка на `/report/:id` отдаёт 404** (D-098): маршрут динамический, а корневой `404.html` отключает SPA-фоллбек Pages и перехватывает путь раньше `_redirects`. Свежий сабмит работает (клиентская навигация), присланная/переоткрытая ссылка — нет, что ломает обещание «reachable only through its private link» | backend + frontend | **done** (D-103, 2026-08-09: `functions/report/[[path]].js` + `dist/report-shell.html`, подтверждено на preview-деплое — HTTP-уровень 200 на мусорный id и на реальный скан, noindex сохранён, 404 для настоящих несуществующих страниц не задет. Монтирование React на клиенте НЕ проверено живым браузером — сетевой барьер песочницы, тот же что у Turnstile в A1-LANDING; обоснование через чтение исходника vite-react-ssg. Владельцу стоит один раз открыть реальный отчёт в своём браузере после деплоя) |
...
| A1-SCAN-BUSY-RETRY | Найдено живой серией D-110: при заполненном лимите параллельных браузеров Browser Rendering `puppeteer.launch` кидает 429 → консьюмер мгновенно пишет failScan (`internal`), хотя через полминуты скан прошёл бы. Сделать retry() с задержкой на launch-429 (и, возможно, на «Target closed» до первой страницы) вместо мгновенного отказа; учесть max_retries и не пересканировать уже начатое | backend-engineer | D-110 | **done** (D-112 + D-113, 2026-08-10: приоритет поднят живым инцидентом — исчерпана суточная квота Free-тарифа, сканер лежал для всех; busy-retry 20с/40с + errorCode `busy` + честный текст на фронте; «Target closed» сознательно НЕ включён в busy. Задеплоено (воркер `de18d14f`), владелец перешёл на Workers Paid. **Подтверждено живьём**: до апгрейда — busy-retry реально отработал на проде (errorCode=busy, отказ через 64–70с = расписание 20+40с); после апгрейда — серия 3 сканов en.zebrakita.de подряд, все три done (66/38/43с, 6 стр., 128 находок)) |
...
**Критический путь развёрнут владельцем 2026-08-15 (D-174):** cold start
двустороннего маркетплейса — сначала спрос (реальные лиды через
`/request-quote/`, сейчас 0), только потом outreach/claim/доставка. `A2-OUTREACH-
SEND`, `A2-STRIPE-LIVE`, `A2-CLAIM-REBUILD` технически там же, где были (см. их
`notes` в GRAPH.yaml), но явно деприоритизированы до появления объёма лидов —
не начинать раньше, даже когда технические блокеры (домен/Stripe-ключи/CI)
снимутся. Текущий фронтир Фазы 2 — Growth-узлы ниже (`G-INTERLINK-AUDIT`,
`G-INDEXNOW`), они двигают именно спрос.
...
## Фаза R — Надёжность и автоматизация (сквозная, D-173, 2026-08-15)

Заведена по прямому запросу владельца: «чтобы меньше всего нужно было проверять
вручную». Принцип узла — **каждая проверка, которую сегодня делает человек
глазами или curl'ом, должна стать автоматической и уметь провалиться**
(доказывается канарейкой, Шаг 4B). Полное описание и ценность каждого — в
`ROADMAP.md` §Фаза R.

| ID | Задача | Владелец | Статус |
|---|---|---|---|
| `R-SMOKE-DEPLOY` | Smoke-тест ЖИВОГО сайта сразу после выката (`scripts/smoke-prod.mjs` + шаг в `deploy.yml`). 12 проверок: страницы по своему маркеру, число агентств из этого коммита, 404 на мусорном пути, адрес воркера в живом бандле, живость воркера, sitemap | devops-engineer | **done** (D-173). Гейт доказан 3 канарейками; первый прогон нашёл баг в самой проверке (regex `src=` vs `rel=modulepreload`). Вступит в силу на следующем деплое |
| `R-HEALTH-CRON` | Ежедневный health-check прода из УЖЕ существующего worker-cron (`0 3 * * *`, `worker/index.js::scheduled`) + письмо владельцу через Resend при поломке. Ловит то, что происходит МЕЖДУ деплоями: протухший токен, упавший воркер, DNS, D1 | backend-engineer | **review** (D-188, 2026-08-22) — код+тесты готовы (4-й `waitUntil`, `worker/lib/healthcheck.js`, D1-состояние `migrations/0012`, alert edge-triggered с идемпотентным маркером, канарейка живая), `worker:test` 542/542, все гейты зелёные; НЕ задеплоено и миграция не применена к прод-D1 — ждёт решения владельца о деплое |
...
| `R-LINKROT` | Плановая проверка `website`/`sourceRefs` 574 агентств на 404/смену домена | data-engineer | **done** (2026-08-18) — `scripts/check-linkrot.mjs`, 83/1694 проблемных URL, отчёт в `docs/project/domains/data.md` |
| `R-STALE-DATA` | Отчёт «N профилей не сверялись > 180 дней» из `lastVerified` | data-engineer | done (2026-08-20) |
...
числился done только здесь и в DECISIONS.md, узла в графе не было). Ни один
узел Фазы R не в фокусе D-174 (не двигает лиды) — безопасные кандидаты для
отдельного захода, когда до них дойдёт очередь.

**Явно НЕ автоматизируем** (сознательное решение, не пробел): выкат сайта и
воркера по расписанию без человека — D-022 требует разрешения владельца на
КАЖДЫЙ выкат; массовые рассылки агентствам — тем более.

## Growth (параллельно, не блокирует)

| ID | Задача | Статус |
|---|---|---|
| `G-INTERLINK-AUDIT` | **Приоритет D-174 (2026-08-15)**: аудит внутренней перелинковки 27 гайдов — SEO-INDEXING-PLAN.md п.6. Двигает спрос к воронке `scan→report→request-quote` без деплоя платных ресурсов | **done** (D-176) — секция «Related guides» на каждом гайде (standard→country приоритет, лимит 4), 6 новых тестов на реальных данных каталога. Не задеплоено |
| `G-INDEXNOW` | **Приоритет D-174 (2026-08-15)**: IndexNow-пинг Bing/Yandex при деплое — SEO-INDEXING-PLAN.md п.5 | **done** (D-178) — scripts/indexnow-ping.mjs + шаг в deploy.yml, весь sitemap на деплой, best-effort. Не задеплоено, реальный первый пинг — на следующем деплое |
| `G-CHECKER-INTERLINK` | Перелинковка 6 чекеров между собой — тот же дефект «звезды», что D-176 у гайдов, только хуже: 5 из 6 вели на contrast-checker, а он сам — ни на один | **done** (D-179) — общий реестр `src/lib/checkers.ts` + блок «Other free checkers» на всех 6 страницах, 6 новых тестов. Не задеплоено |
| `G-CHECKER-STATEMENT-GEN` | **Кандидат №1 по исследованию рынка (D-179, growth.md)**: Accessibility Statement Generator, форма → готовый HTML. Самый плотный кластер конкурентов (8+ игроков), максимальная близость к воронке | **done** (D-181) — `/checkers/accessibility-statement-generator/`, 15 тестов; ключевое: `compliant` (EU) vs `conformant` (W3C) выбирается по рамке, прецедент D-170. Не задеплоено |
| `G-CHECKER-HTML-PARSER` | **Кандидаты №3-4 (D-179)**: Alt Text Checker + Heading Structure Analyzer на общем DOMParser-движке (вставка HTML — ввод URL невозможен на клиенте из-за CORS). Heading даёт двойной кластер a11y+SEO | **done** (D-183) — `htmlAudit.ts` ядро (16 тестов) + `DOMParser` обёртка + общий `HtmlAuditTool` (2 URL). Живая Playwright-проверка. Не задеплоено |
| `G-CHECKERS-ELEVATION` | Дизайн-проход по 9 бесплатным чекерам `/checkers/` — владелец: «лёгкие и непривлекательные, нужно качество уровня Apple». Диагноз: `.card` поднимал тень только на `:hover` (на мобильном — плоско всегда), в `@theme` не было ни одного shadow-токена, инструмент визуально сливался с соседним абзацем статьи | **done** (2026-08-20) — 4 новых `--shadow-*` токена + `--color-surface-elevated`, `.card` с тенью в покое, новые классы `.panel`/`.result-hero` применены во всех 9 чекерах, ритм отступов выровнен. Разбор — `domains/design.md` §3. Все гейты зелёные (audit-a11y ×3 подряд 71/0). Не задеплоено |
| `G-CHECKER-IMAGEPICKER` | Десятый бесплатный чекер — image colour picker (D-182/D-186: крупнейший подтверждённый поисковый объём среди всех кандидатов-чекеров, 49 500/мес США по каждой из двух точных фраз). Продуктовая рамка: не клон universal picker — каждый выбранный цвет обязан показывать контраст с белым/чёрным и вердикт AA/AAA, мост в contrast checker | **done** (2026-08-20) — `/checkers/image-color-picker/`, `src/lib/colorPicker.ts` (5 чистых функций, 19 тестов), полноценная клавиатура (стрелки/Shift/Enter/Space), два aria-live региона. Два бага пойманы и починены живым измерением до завершения узла: гидрация canvas-ref и сгиб 390×844. Разбор — `domains/frontend.md`, узел `G-CHECKER-IMAGEPICKER`. Все гейты зелёные (src:test 210/210, audit-a11y ×3 подряд 72/0). Не задеплоено |
| `G-SCAN-REPORT-ELEVATION` | `/scan/` и `/report/:id` — последние две страницы на старой плоской системе (`.card`, ни одного `.panel`/`.result-hero`) после того, как её получили все 10 чекеров | **done** (2026-08-20) — `/report/:id`: score-блок → `.result-hero` (один на страницу), «Remediation estimate» + тизер платного плана → `.panel`; список находок и правовой блок сознательно не тронуты (уже своя тень / язык статьи, не инструмента). `/scan/`: форма → `.panel`, `.result-hero` не добавлен (страница ввода, ответа нет). `check-fold` расширен на `/report/:id` (5/5, было 4/4); фикстура вынесена в `scripts/lib/report-fixture.mjs`, переиспользуется `audit-own-a11y.mjs`. Сгиб score: 710px → 734px, оба в пределах 844px. Разбор — `domains/frontend.md`, узел `G-SCAN-REPORT-ELEVATION`. Все гейты зелёные (audit-a11y ×3 подряд 72/0, байт-в-байт то же число до/после рефакторинга фикстуры). Не задеплоено |
| G-VPAT-SERVICE | **Кандидат из конкурентного анализа UserWay (2026-08-11)**: у них VPAT/ACR-документация продаётся как B2B-услуга для закупок; у нас уже есть гайд `vpat-en-301-549-mapping-guide` и маппинг EN 301 549 — оформить помощь с ACR как услугу/лид-категорию (естественное расширение, не новый контент). Масштаб и формат не оценивались — нужен scoping-разговор | todo (кандидат, решение владельца) |
| G-I18N | Интерфейс en/de/fr/pl + hreflang | **hreflang закрыт как неприменимый (D-102, 2026-08-08)**: сверка всех 24 гайдов по локалям показала, что переводных пар нет вовсе — fr/pl/de-гайды каждый про свой национальный закон (RGAA/ustawa/BFSG), не перевод друг друга; `alternate` тут был бы ложным сигналом поисковику. `<html lang>` уже верен per-странице — этого достаточно. **Chrome-перевод en/de/fr/pl done** (D-087 + D-102, узел `G-I18N-CHROME`) — см. ниже. Остаток (полный роутинг `/de/`, перевод контента на es) — отдельные будущие решения, каждое требует нового scoping-разговора |
...
| G-I18N-CHROME | Продолжение G-I18N-DE: словарь на fr/pl (2+1 гайда жили в английском chrome) + найденный попутно дефект — сама страница гайда (`GuidePage.tsx`: «Updated»/«FAQ»/«Ready for the next step?»/«Verified agencies for this topic», крошка «Knowledge») не переводилась вообще, даже для уже готового немецкого chrome | frontend-engineer | **done** (D-102, 2026-08-08: `ChromeDict` расширен ключом `guide`, заполнен на всех 4 локалях; польский `plAgencyNoun` меняет форму по числу (245→dostawców, 243→dostawcy); французская типографика — узкий неразрывный пробел перед „;“/„?“, неразрывный внутри «guillemets». Постоянный гейт `src/lib/i18n.test.mjs` (полнота ключей + анти-протечка английских строк + число→форма) — все 3 канарейки подтверждены (каждая роняет ровно свой тест). `audit-own-a11y.mjs` слепо проверял только английские гайды — добавлены de/fr/pl-представители, канарейка (задвоенный aria-label) подтвердила, что гейт реально ловит нарушение. dist-diff: 24/452 файла отличаются — ровно гайды, дифф построчно вычитан (English без изменения видимого текста, de/fr/pl — переведённая обвязка)) |
