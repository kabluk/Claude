...
| G-FOUNDED-LI | Перепроверить записи без `founded` глубже (Impressum, торговый реестр, страница истории), не только homepage+About. Пробный пакет 20/180 закрыт: 6 добавлено, 1 отклонён (недостоверный источник), 13 подтверждены «нигде не публикуется». Список 72 «только LinkedIn» из D-047 не сохранился как данные — заменён прямым срезом `founded == null` (180 записей) | data-engineer | G-FOUNDED | **partial** (D-051; 71/245, остаток очереди 160) |
