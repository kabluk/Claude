...
| G-GUIDES | Новые гайды под кластеры BFSG/EAA/VPAT | **in progress** (D-056, D-057, D-059): выбран product-lead как более ценный прямо сейчас (контент растит трафик и без реального домена, hreflang — нет). Первый кусок — **done**: гайд `barrierefreiheitserklaerung-bfsg-anlage3` (`G-GUIDES-BFSG-STATEMENT`). Второй кусок — **done** (D-057, `G-GUIDES-EAA-8`): 8 юрисдикций D-031. Третий кусок — **done** (D-059, `G-GUIDES-VPAT-CLUSTER`): новый гайд `vpat-en-301-549-mapping-guide` связывает VPAT/ACR с EN 301 549/WCAG (факты перепроверены живьём из самих `.docx`-шаблонов ITI и первичного PDF ETSI, ранее не проверялись в этом проекте) и явно отличает VPAT от Barrierefreiheitserklärung/RGAA déclaration со ссылками, не дублируя. Остаток НЕ начат: ещё BFSG-контент (Muster-Vorlage/чеклист) |
