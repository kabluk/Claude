...
| G-GUIDES-RGAA-FR | Кандидат по данным keyword-рефреша 2026-08-08 (`domains/growth.md`): FR-гайд под голое `rgaa` — 8100/мес @ $3.63, информационный интент («что такое RGAA»), largest gap спрос↔контент; существующий `audit-rgaa-guide.md` метит только в `audit rgaa` (390/мес). Требует выбора модели владельцем перед стартом | product-lead | todo (не начат — узел предложен, владелец ещё не запускал) |
