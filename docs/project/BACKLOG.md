...
| A0-SOURCEREF-LANG | У 24 агентств в `sourceRefs[].label` — русские служебные пометки верификации («checked … пакеты 11-15», «по архивной копии…»), и AgencyPage рендерит их пользователю (найдено при D-105). Вычистить мета-комментарии из label (цитаты и даты сохранить), НЕ выдумывая содержимого; проверить и `foundedNote`-подобные поля | data-engineer | — | **todo** — правка данных каталога, перед стартом пауза с выбором модели (CLAUDE.md) |
