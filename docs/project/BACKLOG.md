...
| A0-DEPLOY | Конфиг Cloudflare Pages + деплой из CI по push в `accessatlas` | devops-engineer | A0-ORIGIN | **review** (основная часть **done** — D-094, 2026-08-08: NS переключены владельцем, зона `active`, **сайт живой на https://verscala.com**, 12 ключевых URL 200, почта цела. Ловушка: корневой домен завис в `pending` — HTTP-валидация сертификата стартовала пока зона была `pending`; починено пересозданием кастомного домена. **Осталось только:** автодеплой из CI — деплой пока ручной `wrangler pages deploy dist --branch main`) |
| A0-GSC | Google Search Console: sitemap, мониторинг индексации первую неделю | growth-strategist | A0-DEPLOY | **разблокирован** (D-094: сайт живой на https://verscala.com, sitemap 415 URL отдаётся с боевого домена) |
...
| A1-REPORT-DIRECT-LINK | **Прямая ссылка на `/report/:id` отдаёт 404** (D-098): маршрут динамический, а корневой `404.html` отключает SPA-фоллбек Pages и перехватывает путь раньше `_redirects`. Свежий сабмит работает (клиентская навигация), присланная/переоткрытая ссылка — нет, что ломает обещание «reachable only through its private link». Решение: Pages Functions `functions/report/[[path]].js` (шелл со статусом 200) либо client-only-маршрут с собственным шеллом | backend + frontend | **todo (высокий)** — ломает основной сценарий продукта |
...
| ID | Задача | Статус |
|---|---|---|
| G-I18N | Интерфейс en/de/fr/pl + hreflang | todo — **масштаб оценён 2026-08-08**: полный роутинг `/de/` удвоил бы сайт до ~900 страниц, `hreflang` по-прежнему упирается в домен-блокер (`A0-ORIGIN`/`A0-OWNER-LEGAL`). **Первый безопасный кусок done (D-087, узел `G-I18N-CHROME-DE`)** — см. ниже. Остаток (роутинг, hreflang, перевод контента на fr/pl/es) — отдельные будущие решения, каждое требует нового scoping-разговора |
...
| G-GUIDES-RGAA-FR | FR-гайд под голое `rgaa` (8100/мес, информационный интент) по данным keyword-рефреша 2026-08-08 | product-lead | **done** (D-090, 2026-08-08: `rgaa-guide.md` — définition/106 критериев/13 thématiques/связь WCAG-EN 301 549/RGAA 5 fin 2026; модель Sonnet — выбор владельца; факты перепроверены родительской сессией на первоисточниках DINUM дословно, вкл. «18 mois» — субагент этот факт не декларировал проверенным; полный набор гейтов + разовый axe 0 нарушений; взаимные ссылки с `audit-rgaa-guide.md` в обе стороны) |
...
| G-I18N-DE | Оболочка сайта (шапка/футер/навигация) на `/bfsg-check/` и немецких гайдах пока английская — `<html lang="de">` при английском chrome. Осознанный компромисс до G-I18N, не недосмотр | frontend-engineer | **done** (D-087, 2026-08-08: `src/lib/i18n.ts` — словарь en/de, `Layout.tsx` принимает `locale`; `/bfsg-check/` + 5 немецких гайдов теперь с реальным немецким chrome, 0 новых URL, полный dist/ побайтово идентичен на ~440 остальных страницах — проверено родительской сессией) |
...
| CN-SCAN-PHASES | НАСТОЯЩИЙ пофазный стрим скана (statement/feedback/DOM-checks/axe по страницам): воркер пишет промежуточный прогресс в D1, контракт GET /api/scan/:id расширяется | backend + frontend | деплой воркера — **решение владельца** (D-022) | **done** (2026-08-08, D-088: владелец выдал `CLOUDFLARE_API_TOKEN` in-memory прямо в чат, никогда не записан на диск/в коммит — `worker:deploy` + `db:migrate:remote` выполнены реально на проде, 5 миграций (0003-0007) применены на реальной D1. Проверено живым сканом против `www.w3.org/WAI/` с `countryCode=DE`: реальные фазы discovering→statement→axe→dom-checks видны в GET /api/scan/:id с растущим pagesDone, финал сбросил progress в null; попутно подтверждён живьём D-040 — `jurisdictionNote`/critical impact реально доходят до находки на проде) |
...
| CN-COMPONENTS | Публичная библиотека доступных компонентов (§22) — новая крупная поверхность, backlink-движок | frontend-engineer | CN-TOKENS | **done** (2026-08-08, D-068: ПЕРВЫЙ КУСОК — каркас `/components/` + `/components/[slug]` из `components.json` (конвенция) + 3 из 13 реально доступных компонента: Accordion/Tabs/Modal; собственные примеры в постоянном audit-a11y, открытая модалка тоже; Playwright 20/20; гейт поймал 3 реальных нарушения и они починены. Остаток закрыт CN-COMPONENTS-REST) |
| CN-COMPONENTS-REST | Остаток библиотеки §22 — по одному образцовому за раз, страница только когда живой пример реален и проходит тот же a11y-порог. **10/10 done: Toast (D-070), Tooltip (D-074), Breadcrumbs (D-077), Combobox (D-078), Menu button (D-079), Select/listbox (D-080), Form field (D-081), Switch (D-082), Pagination (D-083), Data table (D-084, 2026-08-08)**. Библиотека §22 полностью — **13/13 готовых компонентов** | frontend-engineer | CN-COMPONENTS | **done** (10/10, закрыт) |
| CN-RESEARCH | Регулярные data-продукты (§23, §45) на уникальных данных — из САМОГО каталога, не бенчмарк чужих сайтов (корпуса сканов нет, D-010). **Первый кусок done (D-071): `/reports/verified-audit-market`** (agencies.json). **Второй кусок done (D-085): `/reports/en301549-automation-coverage/`** (en301549-coverage.json). **Третий кусок done (D-086, 2026-08-08): `/reports/jurisdiction-coverage-gap/`** — синтез agencies.json + worker/lib/jurisdiction.js (не новый источник, новый угол): 0 из 13 юрисдикций с нулевым покрытием, DE максимум (42), FI/NO минимум (по 2); `scripts/jurisdiction-report-data.mjs`, гейт 7 тестов. Все три прежних отчёта не изменились ни разу (проверено побайтово на каждом шаге). Деплой НЕ требуется | growth + data | — | in progress (3 отчёта; следующий — новый data-продукт по решению направления) |
