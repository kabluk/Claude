...
| G-CERT-EVIDENCE | Перепроверить `evidenceUrl` всех 90+ записей `gov-declared-auditor` и развести «декларация госоргана» и «декларация частной компании» отдельным признаком. Найдено при DE-ENTRY: подпись бейджа утверждала «government», а часть источников — частные сайты (`mazda.de`, `nanu-nana.at`, `stiftung.adac.de`, `pflege-und-zuhause.de`); подпись уже приведена к доказуемому, но сам признак пока один на оба случая. Отдельно: у `ifdb` заявлена `country:'DE'`, а декларация австрийская (`nanu-nana.at`) | data-engineer | todo |
