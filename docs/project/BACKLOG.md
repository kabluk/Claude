...
| G-FOUNDED-LI | Перепроверить записи без `founded` глубже (Impressum, торговый реестр, страница истории), не только homepage+About | data-engineer | G-FOUNDED | **partial** (D-052; 92/245, остаток очереди 139 — 59 проверены и отклонены, 80 не проверены из-за сбоя API-сессии на 5/10 пакетов) |
