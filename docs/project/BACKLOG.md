...
| CN-SCAN-PHASES | НАСТОЯЩИЙ пофазный стрим скана (statement/feedback/DOM-checks/axe по страницам): воркер пишет промежуточный прогресс в D1, контракт GET /api/scan/:id расширяется | backend + frontend | изменение контракта API (INTERFACES.md §3) + деплой воркера — **решение владельца** | todo (D-064; UI-каркас готов — шаги добавятся в ScanStream) |
