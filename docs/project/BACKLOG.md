...
## Сканер: разрыв с тем, что реально проверяют регуляторы (research 2026-08-06)

Ресёрч по запросу владельца, **не реализовано, требует приоритизации**. Вывод:
наш сканер измеряет не то, с чего начинают надзорные органы.
...
| ID | Задача | Приоритет |
|---|---|---|
| A3-STATEMENT | Детект заявления о доступности: шаблоны ссылок/страниц (de/en/fr/nl/pl), достижимость с главной, наличие 4 пунктов Anlage 3, указание надзорного органа. Бинарно, без ложных срабатываний | **высокий** — юридически решающее, полностью автоматизируемое, отсутствует |
| A3-FEEDBACK | Доступный канал обратной связи (EAA требует минимум один) | высокий |
| A3-PDF | Детект ссылок на PDF (в scope EAA, почти всегда недоступны) | средний, тривиален |
| A3-REFLOW | Reflow на 320px (WCAG 1.4.10) — viewport + проверка горизонтального скролла. axe не умеет, а Puppeteer у нас уже есть | высокий |
| A3-KEYBOARD | Клавиатурная навигация: прогон Tab, `document.activeElement`, focus trap, невидимый фокус | высокий |
| A3-MEDIA | Автовоспроизведение (1.4.2), `<video>` без `<track kind="captions">` | средний |
| A3-RESIZE | Увеличение текста 200% (1.4.4) — обрезка/наложение | средний |
| A3-PAGESELECT | Выбор страниц: сейчас первые 5 ссылок с главной (`axe.js:43`). Жалобы и надзор бьют по транзакционным флоу (корзина, вход, форма) — EAA прямо про e-commerce. Приоритизировать страницы с формами | высокий |
| A3-COOKIEBANNER | Cookie-баннер перекрывает страницу → axe видит не тот DOM. Классический ложноотрицательный, ровно как file:// в D-014 | высокий |
| A3-JURISDICTION | Взвешивать severity по юрисдикции, не только по axe impact (страны в каталоге уже есть). «Нет заявления → §37 BFSG, до €100k» ≠ «color-contrast: serious» | средний |
