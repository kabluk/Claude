...
| CN-SCAN-PHASES | НАСТОЯЩИЙ пофазный стрим скана (statement/feedback/DOM-checks/axe по страницам): воркер пишет промежуточный прогресс в D1, контракт GET /api/scan/:id расширяется | backend + frontend | деплой воркера — **решение владельца** (D-022) | **review** (2026-08-07, D-067: код готов целиком — контракт §3/§4, миграция 0007, тесты 211/20, живой прогон wrangler dev --local с фазами в D1; **ждёт worker:deploy — явное решение владельца**; до деплоя на проде старое поведение, UI работает с обоими) |
