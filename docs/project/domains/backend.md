...
Обновлено: 2026-08-11 · Владелец: backend-engineer
...
## A1-SCAN-BUSY-RETRY — 429 от Browser Rendering больше не «сломалось у нас»

**Инцидент (прод, 2026-08-10).** `puppeteer.launch(env.BROWSER)` вернул дословно
`Unable to create new browser: code: 429: message: Rate limit exceeded`.
Лимиты платформы на Free: 1 новый браузер / 20с, 3 параллельных, 10 минут в
день (владелец переходит на Paid — 1/сек, но 429 остаётся возможным). Консьюмер
мгновенно писал `failScan(errorCode='internal')`, и пользователь читал
«Something went wrong on our side» — ложь: у нас ничего не сломано, и с его
сайтом всё в порядке. Занятая платформа — состояние «сейчас нельзя, попробуй
позже», и оно обязано лечиться само, а не превращаться в отказ.

**Классификация (`worker/lib/errors.js`).** Новый код `busy`, паттерн стоит
ПЕРВЫМ в `PATTERNS` — сообщение содержит и `429`, и слова, на которые могут
позариться будущие паттерны; порядок фиксирует, кто выигрывает, до того как
пересечение появится.

    /Unable to create new browser|code:\s*429|Too many (?:concurrent |active )?browser sessions|browser session limit/i

Два сознательных отказа от охвата:
- *не голый `/429/`* — 429 бывает частью URL сканируемого сайта
  (`https://example.com/page429`), а URL попадает в текст почти любой ошибки
  навигации; голый матч уводил бы честный `unreachable` в `busy`. Матчим 429
  только с его префиксом `code:`;
- *не отдельные `Rate limit exceeded` / `Too Many Requests`* — так отвечает и
  САМ сканируемый сайт, когда лимитирует нас, а это по смыслу `blocked`.
  Сказать «наш сканер занят» про чужой рейт-лимит дороже, чем не поймать
  какую-то из формулировок Cloudflare.

**Ретрай с задержкой (`worker/lib/scanJob.js`).** В catch-ветке: если
`classifyError` дал `busy` и доставки ещё остались — `msg.retry({ delaySeconds })`
и новый исход `'busy-retry'`; иначе всё как раньше (`failScan`, теперь с честным
`errorCode='busy'`).

- `MAX_DELIVERIES = 3` — `msg.attempts` в Cloudflare Queues считает ДОСТАВКИ и
  начинается с 1, поэтому `max_retries: 2` даёт три доставки. Число дублирует
  конфиг (в сообщении его нет, вывести в рантайме неоткуда) — и потому закрыто
  гейт-тестом, см. ниже.
- `BUSY_RETRY_DELAYS_SECONDS = [20, 40]` — задержка по номеру текущей доставки.
  Арифметика упирается в окно реапа D-109: `running` закрывается на чтении через
  120с сторожа + 60с grace = 180с от `created_at` (ставится в POST). Доставка 1
  ~0с (busy приходит почти сразу: `launch` падает ДО сторожа и до единой
  навигации) → +20с; доставка 2 ~20с → +40с; доставка 3 стартует ~60с, на полный
  сторожевой скан остаётся ровно 120с — впритык, но реап и не обязан ждать:
  `completeScan` без status-гейта перезапишет реапнутую строку настоящим
  результатом (D-110). Если и третья доставка busy — `failScan` пишется на ~60с,
  с запасом. Третья задержка (80с+) увела бы старт за окно, и пользователь
  получал бы `timeout` от сторожа вместо честного `busy`.
- **В busy-ветке в D1 не пишется НИЧЕГО**, включая прогресс. Строка остаётся
  `running`, и идемпотентный гейт следующей доставки пропустит сообщение именно
  поэтому. Любая запись здесь была бы либо затиранием, либо новым состоянием,
  которое гейт читать не умеет.
- Отсутствующий/мусорный `attempts` трактуется как «последняя доставка» (сразу
  `failScan`), а не как первая. Лишний `retry()` сверх `max_retries` платформа
  просто отбросит: сообщение исчезнет, исход в D1 не запишется, и строку закроет
  реап ошибкой `timeout` — ложь вместо `busy` плюс потерянные 3 минуты. Честный
  отказ сейчас безопаснее потерянного скана.

**Контракт.** `ScanErrorCode` в `src/lib/scanner.ts` и enum в `INTERFACES.md §3`
получили `busy`; текст пользователю — «Our scanner is at capacity right now
because of high demand — nothing is wrong with your site. Please try again in a
few minutes.» (без цифр лимитов: они меняются вместе с тарифом).

**Гейт синхронизации конфига.** Тест в `scanJob.test.mjs` парсит `wrangler.jsonc`
(посимвольный съём комментариев со знанием о строковых литералах — `//` внутри
строки не должен съедать остаток) и сверяет: `MAX_DELIVERIES === 1 +
max_retries`, `BUSY_RETRY_DELAYS_SECONDS.length === max_retries`, сумма задержек
< 180с. Консьюмер ищется ПО ИМЕНИ очереди продюсера, а не по индексу — тогда и
переименование очереди на одной стороне краснеет. Без гейта дубликат разъедется
молча в обе стороны: подняли `max_retries` — busy перестанет ретраиться на
последних доставках; опустили — ретраим в пустоту, теряя исход.

**Канарейки.**
- (A) `code:\s*429` → голый `429`: краснеет «429 inside a scanned URL is NOT busy».
- (B) паттерн `busy` удалён: краснеют оба теста на прод-сообщение.
- (C) `BUSY_RETRY_DELAYS_SECONDS = [20,40,80]` + снят потолок по attempts:
  краснеет «busy на последней доставке» (ретрай вместо failScan) И гейт конфига.
- (D) задержка всегда 20с: краснеет «busy на 2-й доставке — 40с».
- (E) `msg.retry()` без опций: краснеют оба теста на расписание.
- (F) `MAX_DELIVERIES = 4`: краснеет только гейт — ровно тот дрейф, ради
  которого он написан.
- (G) в busy-ветку добавлен `failScan` перед `retry`: краснеет «строка в D1 НЕ
  тронута».
- Честно: одиночная проверка `attempts >= MAX_DELIVERIES` избыточна — её
  дублирует длина массива задержек, и канарейка краснеет только при поломке
  обеих. Оставлена намеренно как декларация намерения.

242/242 `worker:test` (было 233), 34/34 `src:test`, `typecheck` чистый.

**Не проверено.** Живое поведение: что Cloudflare действительно уважает
`delaySeconds` на этой очереди, и полный ли список формулировок 429 у Browser
Rendering — дословно наблюдалась ровно одна строка, остальные альтернативы
паттерна написаны по смыслу и живым текстом НЕ подтверждены (сеть в этой задаче
запрещена). Если прод отдаст другую формулировку, она снова упадёт в `internal`
— и это видно по `scans.error`, где лежит сырой текст.

## SCAN-RESILIENCE (D-113) — «одна плохая страница не убивает скан»

Пять изменений в `worker/lib/axe.js` (полные обоснования — в комментариях кода,
это конспект для навигации):

1. **`gotoSettled()`** — критерий готовности `domcontentloaded` + успокоение
   сети сверху и с потолком (`SETTLE_MS` 4с, шов `env.SETTLE_MS`) под СВОИМ
   таймером (урок D-108: `.catch()` не ловит зависший промис).
   `networkidle0` удалён отовсюду: сайты с непрерывной телеметрией не
   затихают никогда — это был гарантированный NAV_TIMEOUT на весь скан.
2. **`scanOnePage()`** возвращает урожай, зовущий вливает только при успехе.
   Отказ подстраницы (навигация с одним повтором `NAV_RETRIES=1` / инжект axe /
   evaluate) → `scan-meta-page-skipped` (impact minor, причина в html),
   отказ главной — фатален, как раньше. Пропущенные страницы не попадают в
   pageDocs site-checks. `currentUrl` отслеживает реальную позицию страницы —
   это закрыло попутный баг: неудачный возврат со страницы заявления молча
   оставлял axe сканировать её под видом главной.
3. **Один перезапуск браузера** (`MAX_BROWSER_RESTARTS=1`) на смерть сессии:
   `SESSION_DEAD_RE` выписан ДОСЛОВНО из исходника @cloudflare/puppeteer 1.3.0
   (тест-гейт держит список при обновлении зависимости), плюс
   `browser.isConnected()===false`. Страница пересоздаётся целиком
   (`preparePage()`); падение перезапуска не глушится → busy-ретрай D-112.
   Гонка со сторожем закрыта флагом `scanAborted` (новый браузер, созданный
   после срабатывания сторожа, закрывается немедленно).
4. **Перехват запросов**: блок `media` + `TRACKER_HOST_BLOCKLIST` (11 доменов),
   матч по hostname на границе поддомена; документ верхнего уровня — никогда
   (скан сайта самого трекера не должен читаться как 'blocked'). Каждый запрос
   гарантированно получает вердикт; установка фильтра best-effort — поддержка
   `setRequestInterception` в Browser Rendering живьём НЕ проверена.
5. **`keep_alive: 180000`** при launch — дефолтная жизнь сессии 60с меньше
   сторожа 120с; разрыв — вероятный источник «Target closed» посреди скана.

**score**: `scan-meta-*` исключены из штрафа (`score.js::isMetaFinding`) —
раньше `scan-meta-cookie-banner-dismissed` втихую стоил сайту 1 балл, а новый
пропуск страницы штрафовал бы сайт за отказ НАШЕГО сканера. Зеркало инварианта
во фронте — `costEstimate.ts::SCAN_META_PREFIX` (уже был).

**Проверка**: worker:test 255/255; 14 канареек субагента (ключевая C12:
«сочинённый по смыслу» паттерн смерти сессии красен против списка реальных
строк библиотеки — две выдуманные строки, одна упущенная реальная); канарейка
родительской сессии нашла пробел (снятие сброса `currentUrl` в
`restartBrowser` не роняло ни одного теста → молчаливый скан about:blank под
видом реальной страницы) — закрыт 255-м тестом, доказан той же мутацией.

**После деплоя проверить живьём**: 3 скана en.zebrakita.de подряд; сайт с
непрерывной аналитикой (успокоение сети вместо таймаута); что перехват
запросов не подвесил страницы (если Browser Rendering его не поддерживает —
фильтр молча не встанет, скан пойдёт без него). Наблюдаемость (число
пропусков/перезапусков в таблице scans) — кандидат в отдельный узел.

## A2-PDF-PLAN — `GET /api/scan/:id/pdf`, реализован (2026-08-10)

Первый кусок воронки монетизации (владелец, разговор 2026-08-10): PDF-план
исправлений, товар за €19.99. Оплаты/писем/пейволла в этом узле НЕТ по прямому
решению владельца — только сам документ (обоснование: если документ не стоит
денег, платёжная обвязка бессмысленна). Роут открыт, но структурирован так,
что гейт доступа мог бы встать одной проверкой прямо перед `generatePdf()` в
`worker/routes/scanPdf.js::handleGetScanPdf` — токенов/таблиц покупок не
изобретено, это следующий узел.

**Файлы**: `worker/lib/pdfPlan.js` (чистая сборка данных плана — без браузера,
без D1), `worker/lib/pdfPlanHtml.js` (чистый рендер HTML из данных плана, без
браузера), `worker/routes/scanPdf.js` (D1 + юрисдикция + печать через Browser
Rendering), `worker/lib/costEstimate.js` и `worker/lib/oursDescriptions.js`
(вынужденные зеркала фронтенда, см. ниже), `worker/index.js` (маршрут
`GET /api/scan/:id/pdf`, добавлен ПЕРЕД общим `GET /api/scan/:id` — иначе
`startsWith('/api/scan/')` проглотил бы путь как `id === "<uuid>/pdf"`, тихий
404). **`INTERFACES.md` §2 не обновлён этим агентом** (правило `backend-
engineer`: не редактировать `docs/project/*.md` — за родительской сессией
добавить строку `GET /api/scan/:id/pdf | — | application/pdf | 1 | ✅
реализован`).

### Источник каждого блока документа (железное правило узла — ничего не выдумывать)

1. **Приоритеты** — `findings` из D1-записи скана, дедуп по `ruleId` (худший
   impact среди инстансов, тот же принцип, что `score.js`, но НЕ импортирован
   оттуда — `score.js` не экспортирует группировку, только сам score;
   `isMetaFinding` ИМПОРТИРОВАН напрямую из `score.js`, не задублирован).
   Сортировка: severity → число различных страниц с этим правилом → число
   инстансов → `ruleId` (стабильный tie-break).
2. **ruleId → WCAG SC → EN 301 549 clause → title** — индекс строится ОДИН раз
   из `data/a11y/en301549-coverage.json` (`import ... with {type:'json'}`, тот
   же приём, что `matchAgenciesServer.js`), по `axeRules[]` И `ours`. Реальный
   axe-ruleId (пример в тесте — `color-contrast`) резолвится; неизвестный
   (реальный пример, а не синтетика — `landmark-one-main`, axe best-practice
   правило БЕЗ тега WCAG SC, подтверждено чтением метаданных установленного
   `axe-core`: из 105 правил 75 с wcag-тегом, `coverage.json` мапит 71 из
   них) деградирует честно: `kind:'unknown'`, `what: null`, никакого
   выдуманного названия.
3. **Проза для СВОИХ проверок (`a11y-*` из domChecks/siteChecks)** —
   `worker/lib/oursDescriptions.js`, дублирующая копия `OURS_DESCRIPTIONS` из
   `src/lib/wcag.ts` (см. «Дублирование» ниже). Для голых axe-правил прозы НЕТ
   и не сочиняется — показывается только название критерия из coverage.json,
   честно помеченное как «what to fix», не «объяснение» (`describeRule` kind
   `'axe'` несёт caveat об этом же текстом).
4. **4 находки вне гл.9** (`a11y-statement-missing/-incomplete`,
   `a11y-feedback-missing`, `a11y-pdf-present`) — у них нет EN 301 549 clause
   (они из самой директивы/гл.10, см. `src/lib/coverage.ts::beyondStandard`,
   зеркало-константа `BEYOND_STANDARD_INFO` в `pdfPlan.js`, 4 статичные
   строки, гейта нет — низкая частота изменения, тот же класс, что
   `scanner.ts::IMPACT_ORDER`). Текст «что чинить» для них — САМ `finding.html`,
   уже написанный `axe.js` в момент скана («no accessibility statement link
   found on the home page» и т.п.) — не придуман здесь.
5. **Юр-контекст** — `worker/lib/jurisdiction.js` НЕ ТРОНУТ, только вызван.
   Таблица `scans` не хранит `countryCode`, поэтому юрисдикция
   восстанавливается через `resolveJurisdiction(scan.url, overrideCountry)`,
   где `overrideCountry` — это `jurisdictionCountry` первой находки, у которой
   оно есть (ставится ТОЛЬКО на `a11y-statement-missing/-incomplete`, когда
   юрисдикция была решающей на момент скана, D-040). Если такой находки нет
   (заявление было в порядке) — честный откат на TLD-эвристику, то же
   поведение, что у никогда не сканированного URL. Статус заявления
   (`present/missing/incomplete/not-required`) в PDF выводится из РЕАЛЬНЫХ
   findings этого скана, не предполагается по одному факту юрисдикции.
   Сумм штрафов в этом пути нет и не может быть — `jurisdiction.js` их не
   производит вовсе (D-035), и отдельный тест в `pdfPlanHtml.test.mjs` гоняет
   regex по всему рендеру для DE (verified) и FR (unverified).
6. **Оценка усилий** — см. «Дублирование» ниже, всегда с дисклеймером «not a
   quote, an offer, or legal advice» (текст согласован с уже существующим в
   `ReportPage.tsx`, не выдуман заново).
7. **Честность о пропуске страниц** (D-113) — `scan-meta-page-skipped`
   находки НЕ идут в приоритеты/ТЗ (инвариант, дедуп фильтрует ВСЕ
   `scan-meta-*` через `isMetaFinding`), но отдельный блок «Scan coverage»
   перечисляет их с РЕАЛЬНОЙ причиной из `finding.html`.

### Дублирование логики оценки (архитектурное ограничение узла)

`src/lib/costEstimate.ts` — TypeScript во фронтенде. Технически проверено
экспериментом: `wrangler deploy --dry-run` УМЕЕТ забандлить `.ts`-импорт из
воркера (esbuild транспилирует). Но `worker:test` — это голый `node --test`,
не бандлер: тот же файл падает `ERR_MODULE_NOT_FOUND` на вложенном
`import './scanner'` без расширения (стандартный для TS/бандлеров стиль,
несовместимый с резолвером Node ESM даже при включённом по умолчанию в этой
версии Node `--experimental-strip-types`). Вывод: «бандлится» ≠ «тестируется
нашим гейтом» — решает второе. Подробности эксперимента и общий урок —
`LEARNING_LOG.md`, 2026-08-10, «bundler resolution vs. Node ESM resolution».

Решение — по прецеденту `src/lib/jurisdictions.ts` (D-032, там воркер источник
истины, здесь наоборот, но приём тот же): `worker/lib/costEstimate.js` —
дублирующая копия `effortScore`/`estimateCost`/`formatCostEstimate` (пороги
2/5/10→калибровка D-046 те же 4/12/24, `NON_ENGINEERING_RULES` те же 4
ruleId). **Гейт**: `src/lib/costEstimate.workerMirror.test.mjs` — тесто-файл
может жить только на стороне `src/`, потому что только там (`tsx`) в одном
процессе доступны и настоящий `.ts`-модуль, и обычный воркерный `.js`
(`import` плоского JS работает без трансформации). Тест гоняет ОБА
реализации на идентичных синтетических находках (включая ТОЧНЫЕ границы
4/12/24 — первая версия фикстур не содержала ни одной границы и пропустила
намеренную порчу порога до правки, см. «Канарейки» ниже) и требует полного
совпадения `deepEqual`. Аналогично `worker/lib/oursDescriptions.js` (проза
для своих проверок) ↔ `src/lib/wcag.ts::OURS_DESCRIPTIONS`, гейт
`src/lib/wcag.workerMirror.test.mjs` (`deepEqual` всего объекта). Оба гейта —
за пределами буквального `worker/**` scope backend-engineer (лежат в `src/lib/`),
но необходимы: только эта сторона технически способна сравнить обе половины
дублирования — тот же прецедент, что `src/lib/jurisdictions.test.mjs`.

### Браузер и таймаут

`launchBrowser`/`closeBrowserSafely` в `axe.js` приватные (не экспортированы)
и функционально НЕ тронуты (явный запрет узла) — в `scanPdf.js` тот же
ПАТТЕРН переписан заново (тестовый шов `env.__launchBrowser`, ограниченный по
времени `close()` в `finally`), не импорт. `PDF_TIMEOUT_MS = 30000` (свой
сторож, НЕ `SCAN_TIMEOUT_MS`): печать — один `setContent()` без сети и без
навигации плюс один `pdf()`, на порядок легче скана; 30с — щедрый запас
(тестовый шов `env.PDF_TIMEOUT_MS`/`resolvePdfTimeoutMs`, тот же приём, что
`resolveScanTimeoutMs`). `KEEP_ALIVE_MS = 60000` при `launch()`. Шрифты —
только системный стек (`-apple-system, "Segoe UI", Roboto...` + `ui-monospace`
для кода), Geist (брендовый шрифт сайта) сознательно НЕ подключён — он грузится
с внешнего источника на verscala.com, а `setContent()` не должен зависеть от
сети вообще. Заголовок/подвал страницы — `page.pdf({displayHeaderFooter:true,
headerTemplate, footerTemplate})` (отдельные маленькие HTML-строки с inline-
стилями, класс `.pageNumber`/`.totalPages` заполняет сам Chromium) — не CSS
`@page` margin-боксы (Puppeteer их не поддерживает для печати страниц).
`format:'a4'`, поля 90/70/48/48px (верх/низ шире — место под колонтитул).

### Роут — контракты

- Неизвестный id → `404 {code:'not_found'}` (тот же контракт, что
  `handleGetScan`).
- `status:'running'` (включая реап протухшего по D-109-паттерну, переиспользован
  `isScanStale`/`reapStaleScan` буквально, не задублирован) → `409
  {code:'scan_not_ready'}`.
- `status:'error'` → `422 {code:'scan_failed'}`.
- Нет `env.BROWSER` и нет тестового шва → `503 {code:'pdf_unavailable'}` ДО
  всякой работы (тот же паттерн, что `SCAN_QUEUE` в `scan.js`).
- Успех → `200 application/pdf`, `content-disposition: inline;
  filename="verscala-accessibility-plan-<id>.pdf"`.
- Печать упала (таймаут/ошибка Chromium) → `502 {code:'pdf_generation_failed'}`
  с текстом причины — не пустой PDF, не 500 без объяснения.

### Проверки

`worker:test` 296/296 (было 255), `src:test` 39/39 (было 37), `typecheck`
чист, `scripts:test` 48/48, `npm run build` чист (245 агентств, 415 URL, кодировка
ок). `wrangler deploy --dry-run`: бандл 1318.76→1354.77 KiB (+35.9 KiB), gzip
271.84→281.53 KiB (+9.7 KiB) — рост в основном от `en301549-coverage.json`
(~12 KiB) и пяти новых модулей (~37 KiB исходника). Ничего не задеплоено,
ничего не закоммичено.

**Живая проверка (без платного Browser Rendering — явный запрет узла)**:
`wrangler dev --local` + реальная локальная D1 (`db:migrate:local`) —
`GET /api/scan/nope/pdf` → 404, регрессия `GET /api/scan/:id` (базовый роут)
→ 200 после добавления `/pdf`-маршрута перед ним, вставленная вручную `done`
запись → `GET .../pdf` дошёл до реального `generatePdf()`: miniflare сама
скачала локальный Chromium и попыталась его запустить, упала на «Running as
root without --no-sandbox» (ограничение песочницы этой сессии, не
Cloudflare Browser Rendering и не платный вызов) — роут поймал ошибку и
честно отдал `502 pdf_generation_failed` с текстом причины, не упал и не
завис. Это ценный сигнал: весь путь D1→юрисдикция→сборка плана→запуск
браузера реально пройден вживую, отвалилось только на последнем шаге печати
по причине, не относящейся к коду. **Что проверить после реального деплоя**:
`GET /api/scan/:id/pdf` на реально `done` скан с бинднутым `env.BROWSER`
(платный ресурс) — получить настоящие PDF-байты, открыть документ глазами
(шрифты/переносы/колонтитул/номера страниц на реальном многостраничном
отчёте), убедиться что таблица инстансов не разъезжается на длинных
`selector`/`html`.

### Канарейки (сломал → красный → восстановил)

1. `no-fine-amounts`-эквивалент в `pdfPlanHtml.test.mjs`: вписал `(fine up to
   €10,000)` в `renderLegalSection` → тест `no fine amount is ever rendered
   (DE)` красный → убрал → зелёный.
2. Сортировка приоритетов: поменял местами критерии `severity`/`pageCount` в
   `sortByPriority` → тест `severity first, then how many pages` красный →
   вернул порядок → зелёный.
3. Роут-контракт: подменил `409` на `200` для `running` → тест `scan still
   running -> 409` красный → вернул → зелёный.
4. Гейт `costEstimate.workerMirror`: поменял порог `pickBand` `<=4` на `<=3`
   в `worker/lib/costEstimate.js` → ПЕРВЫЙ прогон остался зелёным (фикстуры
   не содержали границы!) — это сам по себе находка, честно исправлена: в
   `FIXTURES` добавлены три набора, дающие РОВНО 4/12/24 (расчётные, не
   угаданные) → повторная порча того же порога → тест красный → вернул порог
   → зелёный. Задокументировано в коде теста, чтобы урок не потерялся.
5. Гейт `wcag.workerMirror`: подменил текст `does` для `a11y-video-no-
   captions` в `worker/lib/oursDescriptions.js` → `deepEqual`-тест красный →
   вернул текст → зелёный.
6. Реальный найденный (не инсценированный) баг: текст «could not be checked»
   в `renderCoverageSection` был разбит переносом строки внутри
   template-литерала (`could\n       not be checked`) — тест на honesty о
   пропущенной странице упал сразу при первом прогоне, не на canary-порче;
   исправлено убиранием переноса. Второй реальный баг того же прогона:
   фикстура роут-теста «scan still running» использовала фиксированную дату
   `2026-08-10T00:00:00Z` — на момент реального прогона (07:56 UTC) она уже
...
## A3-CRON-RESCAN-DELTA — done (D-135), не задеплоено

Узел графа `A3-CRON-RESCAN-DELTA`: еженедельный ре-скан подписанных URL через
СУЩЕСТВУЮЩУЮ очередь + чистая функция дельты для будущего дайджеста.
Тесты: `npm run worker:test` 418 → **472** (+54 новых: 23 `scanDelta.test.mjs`,
15 `subscriptionCron.sql.test.mjs`, 10 `subscriptionCron.test.mjs`, плюс 6
подтестов внутри них); все 418 прежних остались зелёными.
`npx wrangler deploy --dry-run` чист. Деплой НЕ делался (решение владельца).

**Новые файлы:** `worker/lib/scanDelta.js`, `worker/lib/subscriptionCron.js`
(+ три файла тестов). Изменены: `worker/index.js::scheduled`, `wrangler.jsonc`
(только комментарий), `docs/project/INTERFACES.md` §4.

### Расписание: второй cron НЕ заводился (выбор, оставленный D-135 исполнителю)

Остался один элемент `triggers.crons: ["0 3 * * *"]`, и `scheduled` вызывает
`deleteExpiredScans` И `runSubscriptionRescans` — **двумя отдельными
`ctx.waitUntil`**, чтобы падение ре-скана не отменяло retention (гейт-тест
`scheduled(): a broken re-scan never takes retention down with it`). Причина
против отдельного `0 3 * * 1`: недельность обязана считаться от СОБСТВЕННОГО
последнего скана подписки, а подписываются в произвольные дни. Недельный cron
дал бы залп всех подписок в одну ночь (при `max_concurrency: 2` у очереди — это
часы ожидания) и задержку до 6 дней для только что подписавшегося. Ежедневный
тик + фильтр на строку = ровно недельная периодичность на подписчика и
размазанная нагрузка.

### Cadence-фильтрация: колонки времени нет, метка берётся из связанного скана

В `subscriptions` нет `last_rescan_at` — есть только `last_scan_id`. Миграция
ради метки времени не нужна: метка уже лежит в `scans`. Запрос (единственное
место, где это решается):

```sql
FROM subscriptions s LEFT JOIN scans sc ON sc.id = s.last_scan_id
WHERE s.verified = 1 AND s.status = 'active'
  AND (sc.created_at IS NULL OR sc.created_at < ?)   -- ? = now - 7d
ORDER BY COALESCE(sc.created_at, '') ASC LIMIT 25
```

- **`created_at`, а не `completed_at`.** У идущего скана `completed_at` = NULL и
  неотличим от «никогда не сканировали» → подписка попадала бы в выборку каждую
  ночь, пока прошлый скан ещё идёт. Вопрос фильтра — «тратили ли мы Browser
  Rendering на этот URL в окне», а не «получили ли результат»; ответ на него даёт
  момент постановки, одинаковый для успешного, упавшего и зависшего скана.
- **LEFT JOIN, а не INNER.** Скан старше `RETENTION_DAYS` удаляется (D-019), и
  `last_scan_id` повисает на несуществующей строке. INNER JOIN означал бы, что
  такая подписка молча перестаёт сканироваться НАВСЕГДА. 90 дней retention много
  больше 7 дней cadence, лишних сканов это не создаёт.
- **`last_scan_id` перезаписывается сразу после успешной постановки в очередь**,
  не после завершения скана: скан завершится в консьюмере через минуты, а до тех
  пор подписка выглядела бы «никогда не сканированной» (а при падении скана —
  навсегда, то есть ежедневный залп по сломанному сайту). Консьюмер
  (`scanJob.js`) про подписки не знает и знать не должен. Цена названа честно:
  упавший ре-скан = неделя без свежих данных, следующая попытка через 7 дней.
- **Один cutoff на всю выборку** корректен, пока значение cadence ровно одно.
  Держит гейт-тест, разбирающий `INSERT` в `worker/routes/subscribe.js`
  СТРУКТУРНО (позиция колонки в списке → тот же индекс в `VALUES`): наивная
  регулярка `cadence.*'(\w+)'` ловит слово `cadence` из СОСЕДНЕГО комментария и
  осталась бы зелёной при уехавшем литерале — проверено на этом же файле, рядом
  стоит self-check-тест, что гейт действительно краснеет на дрейфе.
- Незнакомая cadence НЕ выбрасывается из выборки (иначе такая подписка перестала
  бы сканироваться молча) — обрабатывается как weekly, то есть в сторону РЕЖЕ,
  с `console.warn`.
- Потолок `MAX_RESCANS_PER_TICK = 25`: каждый ре-скан — оплаченный Browser
  Rendering. Остаток не голодает (`ORDER BY` ставит дольше всех ждавших первыми,
  тик ежедневный).

Семантика выборки проверяется **на настоящем SQLite и настоящих миграциях**
(`worker/lib/subscriptionCron.sql.test.mjs`, прецедент `subscribe.sql.test.mjs`):
фейковый D1 согласован с автором SQL по построению и такой гейт не ловит.
Покрыто в т.ч.: ре-скан вчера → сегодня не due; ровно 7 дней → ещё не due
(строгое `<`); running/error вчера → не due; dangling `last_scan_id` → due;
второй тик того же дня не ставит ничего; отказ очереди закрывает осиротевшую
строку `scans` и НЕ съедает недельный слот; `EXPLAIN QUERY PLAN` показывает
`SEARCH sc` (join по PK), а не полный проход по `scans`.

### Ре-скан идёт тем же producer-путём, что POST /api/scan

`insertScanPending` → `SCAN_QUEUE.send(buildScanJobMessage({id, url,
countryCode: null}))`; отказ `send` → `failScan` (иначе строка `running` висела
бы вечно: реап D-109 срабатывает на GET, а cron-скан никто не читает). Нового
браузерного пути нет. Два намеренных отличия от роута: `scans.email` = NULL
(адрес уже в `subscriptions`, дублировать его в каждую недельную строку — рост
персональных данных без потребителя, R6) и `countryCode` = null (у подписки нет
страны → юрисдикция по TLD).

### scanDelta.js: тождество находки, а не порядок массива

`computeScanDelta(previousFindings, currentFindings, options?)` →
`{new, resolved, scoreChange, scoreBefore, scoreAfter, scopedOutPages}`.

- Ключ — `(ruleId, page, selector)`, склеенные через U+0000; сравнение
  мультимножествами (2 → 1 честно даёт один resolved). Порядок массива не
  участвует вовсе.
- `html` и `impact` в ключ НЕ входят: у axe `html` обрезан до 300 символов, у
  наших проверок это сводка со счётчиками, а `impact` бампит юрисдикционное
  взвешивание — находка не должна «исчезать и появляться» из-за веса.
- **Агрегатные правила**: у `a11y-pdf-present` `selector` = `"3 pdf link(s)"`
  (найдено чтением `axe.js`, а не придумано) — четвёртая PDF-ссылка иначе дала бы
  ОДНОВРЕМЕННО «новую» и «исправленную» находку про одну и ту же непочиненную
  проблему. Для таких правил ключ = `(ruleId, page)`, список
  `AGGREGATE_RULE_IDS` минимальный и явный.
- `scan-meta-*` исключены из new/resolved (как и из score, D-113) — иначе письмо
  «новая находка: cookie-баннер снят перед проверкой» при `scoreChange = 0`.
- Знак: `scoreChange = scoreAfter − scoreBefore`, **плюс = стало лучше**,
  зафиксировано тестом (безнаковое «изменение score» — классический источник
  письма «+7 проблем» на месте улучшения).
- **Живая проверка на РЕАЛЬНО загруженных страницах** (2026-08-11, не на
  собственных фикстурах): находки построены production-кодом (`pdf.js`,
  `statement.js`, `feedback.js`, `siteChecks.js`, `links.js`) по живому HTML
  `bundesregierung.de`, `europarl.europa.eu`, `gov.uk`. Повторный обход того же
  сайта дал ПУСТУЮ дельту (ключи стабильны на реальных URL с query вроде
  `/service/kontakt?view=`), разные сайты — ожидаемые new/resolved.
- Оттуда же — риск, который синтетика бы не показала: набор обойдённых страниц
  между сканами ПЛАВАЕТ (`pickPriorityLinks` берёт ≤6 ссылок по ключевым словам
  из живой главной; сюда же сайты с `;jsessionid=`). Поэтому у функции есть
  необязательный `options.previousPages/currentPages` (из `ScanReport.pages`):
  new/resolved считаются только по страницам, которые обошли ОБА скана,
  остальные уезжают в `scopedOutPages` (их можно честно назвать в письме). score
  всегда считается по полным наборам — он обязан совпадать со `scans.score`.

### Что оставлено A3-CRON-DIGEST-EMAIL (письма здесь НЕТ)

Точный TODO в конце `runSubscriptionRescans`: дельту считать только
`computeScanDelta` + `isEmptyDelta`, пара сканов — `pairs[i].previousScanId` и
`pairs[i].scanId` из возвращаемой сводки, `pages` передавать обязательно.
Отправлять письмо ИЗ ЭТОГО тика нельзя: новый скан в этот момент ещё `running`.
Названо явно, а не умолчано: `last_scan_id` уже перезаписан, поэтому старый id
после возврата из функции в БД не хранится нигде — дайджесту его придётся
