...
`A2-LEAD-EMAIL` **заблокирован отдельно от Resend**: у `Agency`
(`data/a11y/types.ts`) нет поля email вообще — «отправить лид подходящим
агентствам» нечем реализовать, ни у одного совпавшего агентства нет адреса
получателя в каталоге. Это разрыв в модели данных, не техдолг — нужно
решение владельца (добавить email в agencies.json из доверенного источника?
слать только claimed-агентствам, у которых `claims.email` есть, но пока 0
подтверждённых? третий вариант?), прежде чем писать код, который иначе
пришлось бы либо выдумывать данные (нарушая R1), либо оставить нерабочим.
