...
## Конституция → слои (D-062, 2026-08-07)

Владелец передал `DESIGN_CONSTITUTION.md` (корень `docs/project/`). Вердикт
software-architect: стек и слои сохраняются (§62 конституции сам это
требует); меняется подача и добавляются поверхности. Карта:

| Конституция | Текущее состояние | Действие |
|---|---|---|
| §2/§39/§57 без fear-маркетинга | D-035 + гейт `no-fine-amounts` | ничего — уже инвариант |
| §5.1 находка → действие | A1-EXPLAIN/A1-COST/A1-MATCH | ничего |
| §5.5/§48 trust = evidence | sourceRefs/lastVerified/declarant (D-042) | ничего |
| §7-8, §59 scanner-first главная | главная каталог-first | **CN-HERO** (без домена) |
| §6 IA: Scan первым, Knowledge | nav: Countries…Guides + кнопка Scan | **CN-NAV** |
| §9 Score + честные ограничения | score.js + D-030, лимиты названы | trend требует аккаунтов → Фаза 3 |
| §10 deploy-подобный поток скана | текстовый async-прогресс (UX-2) | **CN-SCAN-STREAM** — фазы у воркера уже реальные |
| §11-12 issue-management/kanban | нет (нужны аккаунты) | Фаза 3 (A3-DASH), не раньше |
| §18 RFQ + префилл скана | A2-LEAD-FORM `?scanId=` | ничего |
| §20 /wcag/[criterion] knowledge | en301549-coverage.json есть | **CN-WCAG-PAGES** — реальная фактура, не thin |
| §22 библиотека компонентов | нет | **CN-COMPONENTS**, отдельная фаза |
| §23/§45 data-продукты | уникальные данные есть, корпуса сканов нет | **CN-RESEARCH**, после деплоя |
| §24-29 токены/типографика | Tailwind-дефолт | **CN-TOKENS**, self-host шрифтов |
| §40 WCAG 2.2 AA сам сайт | axe 4.13 (2.2-правила в наличии) | **CN-WCAG22** — проверить конфиг |
| §15-16 поля/фильтры экспертов | данных нет | только с реальными данными (D-045/D-047) |
| §52 платное = помечено | featured | проверить подпись в UI при первом рендере featured |
| §53 pricing-слои | Free scan есть; Monitoring = Фаза 3 | по мере появления продуктов |

Запрещённая интерпретация: конституция НЕ даёт права выдумывать данные под
красивые фильтры/поля (§5.5 сама этого требует) и НЕ отменяет юридические
страницы (§6 допускает их при реальной потребности — Imprint обязателен по
§5 DDG).
