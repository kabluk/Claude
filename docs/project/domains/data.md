...
## R-LINKROT: плановая проверка website/sourceRefs 574 агентств (2026-08-18)

`scripts/check-linkrot.mjs` — read-only HTTP-проверка. Собирает `website`
(домен без протокола, types.ts) и все `sourceRefs[].url` из 574 записей,
дедуплицирует по точной строке URL (**1694 уникальных** из ~2000+ вхождений),
проверяет через `fetch` с общим User-Agent, `redirect:'follow'`,
`AbortSignal.timeout`, ограниченным пулом конкурентности. Классификация по
убыванию «это точно проблема»: `UNREACHABLE` (DNS/сеть/timeout) →
`NOT_FOUND` (404/410) → `DOMAIN_CHANGE` (финальный хост после редиректов ≠
исходному, без учёта `www.`) → `SERVER_ERROR` (5xx) → `BLOCKED` (401/403/429
— чаще анти-бот, не обязательно протухание) → `REDIRECT`/`OK`.

**Итог живого прогона: 83/1694 проблемных URL.**

| Категория | Кол-во | Смысл |
|---|---|---|
| UNREACHABLE | 14 | домен не резолвится/не отвечает — сильнейший сигнал |
| NOT_FOUND | 14 | страница снята (404/410) |
| DOMAIN_CHANGE | 21 | сайт переехал/сменил бренд |
| SERVER_ERROR | 22 | сайт жив, но отдаёт 5xx именно сейчас |
| BLOCKED | 12 | анти-бот (403 Cloudflare и т.п.), не обязательно мертво |
| REDIRECT | 705 | тот же хост, редирект — не проблема |
| OK | 906 | без замечаний |

Заметные находки (не автоправлено, D-047 — это отчёт для ручного review):

- **`website`-поле мертво целиком** (не только отдельный sourceRef) у
  `intopia` (UNREACHABLE — но её же sourceRefs дают 5xx, т.е. сайт
  нестабилен, не обязательно умер), `vision-australia-digital-access`,
  `path-ie`, `inklusio`, `itqcr`, `eleven-ways` (NOT_FOUND), `estart`
  (NOT_FOUND) — кандидаты на приоритетную ручную перепроверку.
- **DOMAIN_CHANGE, подтверждённые реальным ребрендингом/переездом**: `tpgi`
  → `vispero.com` (материнский бренд поглотил дочерний, ожидаемо), `eclusief`
  → теперь бренд «Normeris» на том же домене (see sourceRef label), `perkins-access`
  → `perkinsaccess.org` (тот же субъект, другой TLD/поддомен), `ten10`,
  `nomensa`, `inter.vlaanderen`, `bikosax.de`, `kb-volmarstein.de`,
  `liquid-impressions.com` — везде сайт жив, просто переехал; профиль не
  битый, но `website`/`sourceRefs` стоит обновить на актуальный URL.
  `schalk-and-friends` и `interface-consult` — DOMAIN_CHANGE на СТОРОННИХ
  sourceRefs (декларации муниципалитетов), не на своём сайте — это они
  переехали, не сама декларация.
- **SERVER_ERROR/BLOCKED кластеры на польских/французских записях**
  (`cbi24.pl`, `widzialni.org`, `ads-com.fr`, `access-first.fr`,
  `zenythgroup.com`) вперемешку с анти-бот-паттерном, знакомым по прошлым
  проходам (`level-level.com` — та же Cloudflare challenge, что уже
  фиксировалась в разделе про `founded` выше) — не считать протуханием без
  ручной живой проверки в браузере.
- **UNREACHABLE на сторонних источниках, не на собственном сайте агентства**:
  3 ссылки на `csun.edu/cod/conference/.../exhibitors/view/...`
  (`barrierbreak`, `allyant`, `usablenet`) — программа CSUN Conference 2024,
  вероятно архивирована/переструктурирована организатором; сам факт участия
  не под вопросом, но `sourceRefs.url` как прямое доказательство больше не
  резолвится — заменить на Wayback-копию или другой источник при следующей
  правке этих записей.

Полный построчный список (URL + slug + поле) — в выводе самого скрипта
(`node scripts/check-linkrot.mjs`), не дублируется здесь построчно ради
объёма; таблица выше и заметные находки — компактная выжимка для owner-review.
Канарейка проверки задокументирована в `GRAPH.yaml` (узел `R-LINKROT`,
поле `notes`): скрипт корректно ловит заведомо мёртвый `.invalid`-домен
(`classify()` → `UNREACHABLE`) и уже поймал настоящий случай на живых данных
(`tpgi.com` → `DOMAIN_CHANGE`).

## R-STALE-DATA: отчёт о ре-верификации профилей по lastVerified (2026-08-20)

Инструмент планового поддержания свежести каталога: `scripts/stale-data-report.mjs`
печатает список профилей, у которых `lastVerified` старше порога (по умолчанию
180 дней; `--days N` для другого горизонта, `--json` для машиночитаемого вывода).

- **Живой прогон 2026-08-20: STALE 0/574, invalid 0.** Весь каталог сверен
  2026-08-04…2026-08-13 (волны G-CERT-EVIDENCE/G-PRICE/G-FOUNDED и последующие);
  первые записи пересекут порог 180 дней ~**2027-01-31** — к этой дате отчёт
  начнёт наполняться сам, без правок кода.
- Архитектура как у остальных агрегаторов (reports-data, en301549-report-data):
  чистая `computeStaleReport(agencies, {now, thresholdDays})` без I/O и без
  `Date.now()` внутри — clock подаётся снаружи, тесты детерминированы.
- Строгий парсер даты: `YYYY-MM-DD` + round-trip (отклоняет `2026-02-31`,
  `2026`, число вместо строки). Нечитаемый `lastVerified` — не «протух», а
  дефект данных: отдельный список `invalid` в выводе.
- Exit 0 всегда при читаемом JSON: это отчёт для ручного планирования
  ре-верификации (в духе D-047 и R-LINKROT — ничего не автоправим), не гейт.
- Канарейки прогнаны живьём: подсаженный `lastVerified: 2019-05-01` в реальный
  `agencies.json` пойман (`2668d deque-systems`), откат byte-identical;
  сломанный знак сравнения порога роняет 5/9 тестов. 9 тестов на реальном
  `agencies.json` (D-040), scripts:test 62/62.

Связка с R-LINKROT: linkrot ловит умершие URL, stale-report — устаревшие
проверки фактов. Вместе они дают полный цикл: «что проверить заново» (stale)
и «что уже точно сломалось» (linkrot).
