...
- Данные: 253 агентства; описаний 81/253, но **119 патчей лежат в `data/a11y/enrich/`
  НЕ применёнными** — первый готовый узел A0-ENRICH (один запуск скрипта + валидация).
...
1. `/task-loop A0-ENRICH` — применить патчи описаний (безопасно, verify задан).
2. Спросить владельца: реквизиты Imprint + домен (снимает 3 узла Фазы 0).
3. При approval — `/task-loop A1-SCAN` (сканер MVP, начало Decision Engine).
