...
⚠ **D-073 (тот же день, 2026-08-08): тёмная тема убрана по прямому указанию
владельца.** Раздел 2 ниже (палитра DARK) описывает значения, которые больше
НЕ реализованы в коде — оставлен как исторический источник значений на случай
будущего возврата, но `src/styles.css` их не содержит. Сайт сейчас рендерит
ТОЛЬКО раздел 1 (LIGHT), независимо от системной темы посетителя.
...
## 2. Палитра — DARK (была реализована через `prefers-color-scheme: dark`,
УБРАНА D-073 — не в коде)
...
Правило (D-073): токены объявляются один раз в `@theme` и это единственные
значения, которые рендерит сайт — dark-переопределения нет. Компоненты
по-прежнему стилизуются ТОЛЬКО токенами, не hex/slate-классами напрямую.
...
- **Логотип** — скруглённая плашка `primary` (32px) с БЕЛЫМ stroke-SVG глифом
  ...
  1.75; токены `brand-plate`/`brand-glyph`). Material Symbols из макетов НЕ
  подключается (иконочный шрифт с CDN; §29 — один икон-стиль).
...
`darkMode: class` (D-072 использовал `prefers-color-scheme` вместо
JS-переключателя; D-073 убрал тёмную тему целиком — по прямому указанию
владельца, не как более раннее отклонение от макетов), выдуманная телеметрия
(«SCANNING ENGINE ACTIVE», «99.8% Audit Accuracy», «ACCESSATLAS CORE V2.4.0»)
— против D-035/D-045, счётчиков без источника не показываем; фиктивные числа
макетов (142 агентства, 36 стран) — у нас реальные из сборки. `pitch_deck_*`
(10 макетов) — ОТДЕЛЬНЫЙ артефакт для инвесторов, не сайт: зафиксирован
строкой в `BACKLOG.md`, в D-072 не применялся.
