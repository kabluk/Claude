...
API-ответом. `A2-LEAD-EMAIL` заблокирован отдельно: у `Agency` нет поля
email, слать подходящим агентствам физически нечем — нужно решение владельца
о модели данных. Live Stripe (`A2-STRIPE-LIVE`) ещё не начат/ждёт отдельного
approval. По плану — Фаза 2 (ROADMAP.md).
