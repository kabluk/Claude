...
export type ScanFinding = {
  ...
  // D-131: собственные подсказки axe-core, снятые при скане из того же
  // results-объекта, из которого берутся сами находки (worker/lib/axe.js).
  // help/helpUrl — уровня ПРАВИЛА (одинаковы у всех инстансов одного ruleId),
  // failureSummary — уровня ЭЛЕМЕНТА («Fix any of the following: ...»,
  // реально разный у двух элементов, провалившихся на одном правиле).
  // Есть ТОЛЬКО у находок axe-core и только у сканов после D-131: у наших
  // собственных a11y-*/scan-meta-* проверок и у старых записей D1 их нет
  // вовсе — потребитель обязан работать без них, заглушки не выдумываются.
  // Потребитель сейчас один — платный PDF-план (worker/lib/pdfPlan.js);
  // бесплатный /report/:id их СОЗНАТЕЛЬНО не показывает (D-131: находки
  // бесплатны, потому что доказывают реальность проблем — D-114; инструкция
  // «как это чинить» остаётся тем, за что берут деньги).
  help?: string
  helpUrl?: string
